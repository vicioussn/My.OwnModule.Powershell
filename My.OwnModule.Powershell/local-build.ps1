...
# Clear old packages
Get-ChildItem .\tools\$moduleName\_bin | Remove-Item -Force -Verbose;


#region Get current package version and increment Patch
$version = Select-String -Path .\tools\$moduleName\$moduleName.nuspec -Pattern "<version>(\d)\.(\d)\.(\d{1,})</version>";
...
$a = Get-Content -Path ".\tools\$moduleName\$moduleName.nuspec" | %{$_ -replace "<version>(\d)\.(\d)\.(\d{1,})<\/version>", $nuspecVersionString };
$a | Out-File ".\tools\$moduleName\$moduleName.nuspec" -Verbose;

$a = Get-Content -Path ".\tools\$moduleName\$moduleName.psd1" | %{$_ -replace "ModuleVersion        = `"\d\.\d.\d{1,}`"", $psd1VersionString };
$a | Out-File ".\tools\$moduleName\$moduleName.psd1" -Verbose;
#endregion /Update package version


nuget pack ".\tools\$moduleName\$moduleName.nuspec" -OutputDirectory .\tools\$moduleName\_bin -Properties NoWarn=NU5111,NU5110,NU5100
$package = Get-ChildItem .\tools\$moduleName\_bin\*.nupkg | Sort-Object -Property CreationTime | Select-Object -Last 1;
